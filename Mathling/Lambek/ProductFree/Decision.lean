...
```lean
@[grind]
public def splits {α} : List α → List (List α × List α)
  | [] => [([], [])]
...
```lean
@[grind]
public def picks {α} : List α → List (List α × α × List α)
  | [] => []
...
```lean
@[grind]
public def candidates (Γ : List Tp) : List Cand :=
...
```lean
@[grind .]
public def proveAux : Nat → List Tp → Tp → Bool
  | 0, _,  _ => false
  | n + 1, Γ,  A =>
    match A with
    | Tp.atom s =>
...
```lean
@[important, grind .]
public theorem prove2_iff_sequent : prove2 Γ A ↔ Γ ⇒ A := by grind
```
...
```lean
@[grind .] public def translatedProve1 (toProductFree : α → Tp) (Γ : List α) (A : α) : Bool :=
...
```lean
@[grind .] public def translatedProveAux (toProductFree : α → Tp) (n : Nat) (Γ : List α) (A : α) : Bool :=
...
```lean
@[grind .] public def translatedProve2 (toProductFree : α → Tp) (Γ : List α) (A : α) : Bool :=
...
```lean
@[important, grind .] public theorem translatedProve2_iff_Sequent
    (toProductFree : α → Tp)
    {Γ : List α} {A : α} :
