...
そして、証明探索アルゴリズムが `Mathling.Lambek.ProductFree.Left.Basic` で帰納的に定義された
シーケントの導出と一致することを示す。
...
これは、左除法の右導入規則が **反転可能（Invertible）** であるという性質に基づいている。
...
graph TD
    ...
    Cases -- "左除法" --> Div["右導入規則を適用し<br/>proveAux n ... に再帰"]
...
graph TD
    ...
    Cases -- "右導入規則" --> RightRule["Γ ⇒ A ⧹ B <br/>[A'] ++ Γ ⇒ B へ簡約"]
