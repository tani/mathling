...
# Mathling / Grammar / Conversion / Pushdown モジュール
...
inductive GrammarPDAState where
  | run
  | done
  deriving Repr, DecidableEq

/-- Grammar symbols plus a private bottom marker. -/
inductive GrammarPDAStack (T N : Type*) where
  | bottom
...
def rhsTerminals {N : Type*} : List (Symbol T N) → List T
  | [] => []
...
def terminalSupport (g : ContextFreeGrammar T) : List T :=
  g.rules.toList.flatMap fun r => rhsTerminals r.output

def expansionRule {g : ContextFreeGrammar T}
...
def terminalRule (g : ContextFreeGrammar T) (a : T) :
    PushdownRule T GrammarPDAState (GrammarPDAStack T g.NT) where
  source := .run
  input := some a
...
def finishRule (g : ContextFreeGrammar T) :
    PushdownRule T GrammarPDAState (GrammarPDAStack T g.NT) where
  source := .run
  input := none
  pop := .bottom
...
def toNPDA (g : ContextFreeGrammar T) :
...
def encodeForm {N : Type*} (xs : List (Symbol T N)) :
    List (GrammarPDAStack T N) := xs.map GrammarPDAStack.symbol

def Supported (g : ContextFreeGrammar T) : Symbol T g.NT → Prop
...
def GrammarRunGood (g : ContextFreeGrammar T) (word : List T) :
...
@[important, simp] theorem toNPDA_language (g : ContextFreeGrammar T) :
    (toNPDA g).language = g.language := by
  ext word
  constructor
  · rintro ⟨q₀, hq₀, qf, hqf, stack, hreach⟩
    simp [toNPDA] at hq₀ hqf
    subst q₀
    subst qf
...
noncomputable abbrev toNPDA (g : ContextFreeGrammar T) :=
  _root_.Mathling.Grammar.ContextFreeGrammar.toNPDA g

@[simp] theorem toNPDA_language (g : ContextFreeGrammar T) :
...
@[ext] structure ContextFreeNT (State Stack : Type) where
  source : State
  pop : Stack
  target : State
  deriving Repr, DecidableEq
...
def segmentForm : List State → List Stack →
...
def contextFreeRule (r : PushdownRule T State Stack) (path : List State) :
...
def compatiblePaths [DecidableEq State] (M : NPDA T State Stack)
...
def emptyToContextFreeGrammar [DecidableEq T] [DecidableEq State]
...
theorem mem_listsOfLength {choices path : List State} {n : Nat}
  ...
  | succ n ih =>
      ...
      | nil => simp at hlen
      | cons q path =>
          simp only [List.length_cons, Nat.add_left_cancel_iff] at hlen
...
theorem balanced_derives [DecidableEq T] [DecidableEq State]
  ...
  · intro q hq
    exact ⟨[q], by simp, by simp, by simp, by simpa,
...
mutual
  ...
  inductive SymbolMeaning (M : NPDA T State Stack) :
  ...
  inductive FormMeaning (M : NPDA T State Stack) :
...
theorem contextFreeRule_meaning [DecidableEq State]
  ...
  | some a =>
      simp only [contextFreeRule, hin, Option.toList_some, List.map_cons,
        List.map_nil, List.nil_append] at hmeaning
      cases hmeaning with
...
theorem finalToEmpty_accepts_iff_balanced (M : NPDA T State Stack)
  ...
  · rintro ⟨start, hstart, done, hdone, stack, hreach⟩
    ...
    subst done
    have hstack : stack = [] :=
      M.finalReaches_stack_good hreach (by simp [FinalStackGood, finalToEmpty])
    subst stack
...
def toContextFreeGrammar [DecidableEq T] [DecidableEq State]
...
@[important, simp] theorem toContextFreeGrammar_language [DecidableEq T]
...
def toContextFreeGrammar (M : NPDA T State Stack) : ContextFreeGrammar T := by
  classical
  exact M.toContextFreeGrammar

@[simp] theorem toContextFreeGrammar_language (M : NPDA T State Stack) :
