...
@[grind cases] inductive GrammarPDAState where
  | run
  | done
  deriving Repr, DecidableEq

/-- Grammar symbols plus a private bottom marker. -/
@[grind cases] inductive GrammarPDAStack (T N : Type*) where
  | bottom
...
@[important, grind =, simp] theorem toNPDA_language (g : ContextFreeGrammar T) :
...
@[important, grind =, simp] theorem toContextFreeGrammar_language [DecidableEq T]
...
@[important, grind =] theorem isContextFree_iff_exists_npda {T : Type} {L : Language T} :
    L.IsContextFree ↔
