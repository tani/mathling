...
@[grind cases] inductive LinearGenerates (g : LinearGrammar T) : g.cfg.NT → List T → Prop
...
@[grind cases] inductive LinearPDAState (g : LinearGrammar T) where
  | derive (A : g.cfg.NT)
...
@[important, grind =, simp] theorem toOneTurnNPDA_language (g : LinearGrammar T) :
